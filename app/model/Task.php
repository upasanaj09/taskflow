...
function get_all_tasks_overdue($conn){
    $sql ="SELECT * FROM tasks WHERE due_date < CURDATE() AND due_date!=0000-00-00 AND status!='completed' ORDER BY id DESC";
    $stmt= $conn->prepare($sql);
    $stmt->execute([]);
...
function count_tasks_overdue($conn){
    $sql ="SELECT id FROM tasks WHERE due_date < CURDATE() AND due_date!=0000-00-00 AND status !='completed' ";
    $stmt= $conn->prepare($sql);
    $stmt->execute([]);
...
function count_in_progress_task($conn){
    $sql ="SELECT id FROM tasks WHERE status ='in progress' ";
    $stmt= $conn->prepare($sql);
    $stmt->execute([]);
...
function count_my_tasks_overdue($conn, $id){
    $sql ="SELECT id FROM tasks WHERE status !='completed' AND  assigned_to=? AND due_date  < CURDATE() AND due_date!=0000-00-00 ";
    $stmt= $conn->prepare($sql);
    $stmt->execute([$id]);
...
function count_my_in_progress_task($conn, $id){
    $sql ="SELECT id FROM tasks WHERE assigned_to=? AND status ='in progress' ";
    $stmt= $conn->prepare($sql);
    $stmt->execute([$id]);
